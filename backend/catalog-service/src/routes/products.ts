...
router.get('/', [
  ...
  query('status').optional().isIn(['draft', 'published', 'archived', 'out_of_stock']).withMessage('Status must be draft, published, archived, or out_of_stock'),
...
router.post('/', [
  ...
  body('category').isObject().withMessage('Category must be an object with id, name, and slug'),
