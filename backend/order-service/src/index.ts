...
async function startServer() {
  ...
  try {
    ...
    await connectDatabase(process.env.MONGODB_URI!, 'orders_db');
    ...
    // Subscribe to events
    await kafkaService.subscribeToPaymentEvents();
    await kafkaService.subscribeToInventoryEvents();
    ...
    const server = new ApolloServer({
      ...
      formatError: (error) => {
        logger.error('GraphQL Error in Order Service', {
      ...
      formatResponse: (response, { request }) => {
        logger.info('GraphQL Request in Order Service', {
          query: request.query,
    ...
    app.use((error: any, req: express.Request, res: express.Response, next: express.NextFunction) => {
      logger.error('Unhandled error in Order Service', {
        error: error.message,
        stack: error.stack,
        url: req.url,
        method: req.method,
...
process.on('unhandledRejection', (reason, promise) => {
  logger.error('Unhandled Rejection in Order Service', {
    reason,
    promise,
...
process.on('uncaughtException', (error) => {
  logger.error('Uncaught Exception in Order Service', {
    error: error.message,
    stack: error.stack,
